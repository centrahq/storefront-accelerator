...
import { checkoutQuery, expressCheckoutWidgetsQuery, PaymentConfigResponse } from '../../queries';
...
export const AdyenExpressCheckout = ({ itemId }: Props) => {
  ...
  const initialLineItems = useMemo(() => {
    return checkoutData.lines.map((line) => ({
  ...
  const addressUpdateHandler = async (shippingAddress: google.payments.api.IntermediateAddress) => {
    try {
      ...
      const data = await paymentInstructionsMutation.mutateAsync({
        shippingAddress: {
          address1: '',
          ...address,
        },
  ...
  const handleGooglePayShippingMethodUpdate = async (shippingMethodId: string) => {
    const data = await setShippingMethodMutation.mutateAsync(Number(shippingMethodId));
  ...
  const googlePayPaymentDataChangedHandler: google.payments.api.PaymentDataChangedHandler = useEffectEvent(
    async ({ callbackTrigger, shippingAddress, shippingOptionData }) => {
      if (callbackTrigger === 'INITIALIZE') {
        if (itemId) {
          // Check if product is already in cart
          const hasProductInSelection = checkoutData.lines.some((line) => line?.item.id === itemId);

          if (!hasProductInSelection) {
            try {
              await addToCartMutation.mutateAsync({ item: itemId });
            } catch (error) {
  ...
  const onAuthorizedHandler = useEffectEvent(
    (
      payload: {
  ...
  const handleOnSubmit = useEffectEvent(async (state: SubmitData, actions: SubmitActions) => {
  ...
  const onApplePayClick = useEffectEvent(async (resolve: VoidFunction, reject: (error: Error) => void) => {
    if (itemId) {
      const hasProductInSelection = checkoutData.lines.some((line) => line?.item.id === itemId);
  ...
  const onApplePayShippingMethodSelected = useEffectEvent(
    async (
      ...
      try {
        const data = await setShippingMethodMutation.mutateAsync(Number(event.shippingMethod.identifier));
        const grandTotal =
  ...
  const onShippingContactSelected = useEffectEvent(
    (
  ...
  useEffect(() => {
    ...
    void init();
  }, [initialLineItems, paymentConfig, returnUrl]);
